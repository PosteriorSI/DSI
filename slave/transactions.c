...
uint64_t LoadData(void)
{
	...
	getchar();
	ReadWhse(configWhseCount);

	ReadItem(configUniqueItems);
...
void executeTransactions(int numTransactions, int terminalWarehouseID, int terminalDistrictID, TransState* StateInfo)
{
	...
	{
		//printf("Start transaction %d, index=%d\n", i, index);
		...
		*/
		...
		//printf("End transaction %d, result=%d, index=%d\n", i, result, index);
	}

	//for_test
	StateInfo->extend_abort=td->ExtendAbort;

	//sleep(6);
	//printf("terminal finished index=%d.\n", index);
...
int newOrderTransaction(int w_id, int d_id, int c_id, int o_ol_cnt, int o_all_local, int *itemIDs, int *supplierWarehouseIDs, int *orderQuantities, int* node_id, int node_num)
{
	...
	{
		printf("stmtGetCustWhse not found whse_id=%ld, %d\n", whse_id, node_id2);
		exit(-1);
	}
	//printf("after read.\n");
	//printf("Warehouse: %ld %ld\n",whse_id, whse_value);
	//printf("Data_Read Warehouse: %ld, %ld, %d\n", whse_id, whse_value, node_id2);
	...
	{
		printf("stmtGetCustWhse not found cust_id=%ld, %d\n", cust_id, node_id3);
		exit(-1);
	}
	//printf("Data_Read Customer: %ld, %ld, %d\n", cust_id, cust_value, node_id3);
	...
	while(!newOrderRowInserted)
	{
		...
		{
			printf("stmtGetDist not found  dist_id=%d, %d\n", dist_id, node_id1);
			exit(-1);
		}

		//printf("Data_Read District: %ld, %ld, %d\n", dist_id, dist_value, node_id1);
	...
	for(ol_number=1;ol_number<=o_ol_cnt;ol_number++)
	{
		...
		{
			printf("stmtGetItem not found item_id=%ld, %d\n", item_id, node_id2);
			exit(-1);
		}
		//printf("Data_Read Item: %ld, %ld, %d\n", item_id, item_value, node_id1);
		...
		{
			printf("stmtGetStock not found s_id=%ld,%d\n", s_id, node_id2);
			exit(-1);
		}
		//printf("Data_Read Stock: %ld, %ld, %d\n", s_id, s_value, node_id1);
...
int paymentTransaction(int w_id, int c_w_id, int h_amount, int d_id, int c_d_id, int c_id, int* node_id, int node_num)
{
	...
	if(result == 0)
	{
		printf("payUpdateDist() update failed, dist_id=%ld\n",dist_id);
		AbortTransaction(0);
		return -1;
	}
	*/
	...
	if(c_credit == 0)//bad credit
	{
		...
		{
			printf("payGetCustCdata not found,cust_id=%ld\n",cust_id);
			exit(-1);
		}
...
int deliveryTransaction(int w_id, int o_carrier_id, int* node_id, int node_num)
{
	...
	for(d_id=1;d_id<=10;d_id++)
	{
		...
		if(no_o_id != -1)
		{
			...
			{
				printf("deliveryGetCustId not found,oo_id=%ld\n",oo_id);
				exit(-1);
			}
...
int orderStatusTransaction(int w_id, int d_id, int c_id, int* node_id, int node_num)
{
	...
	if(o_id > 0)
	{
		...
		{
			printf("ordStatGetCustBal not found,oo_id=%ld\n",oo_id);
			exit(-1);
		}
		...
		for(ol_number=1;ol_number<=o_ol_cnt;ol_number++)
		{
			...
			{
				printf("ordStatGetCustBal not found,ol_id=%ld\n",ol_id);
				exit(-1);
			}
...
int testTransaction(int w_id, int d_id)
{
	...
	if(flag==0)
	{
		printf("item table read failed, %ld\n",item_id);
		exit(-1);
	}
	...
	*/
	/*
	result=Data_Update(Item_ID, item_id, item_value);
	if(result==0)
	{
		printf("item table not found,%ld,%ld\n",item_id, item_value);
		exit(-1);
	}
	*/
...
void LoadDataTest(void)
{
	...
	*/
	...
	for(i=0;i<10;i++)
	{
		...
		result=Data_Insert(table_id, tuple_id, value, nid);

		if(result==0)
		{
			printf("abort transaction %d\n", i);
			AbortTransaction();
			return;
		}
	}
	...
	for(i=0;i<5;i++)
	{
		...
		result=Data_Update(table_id, tuple_id, value, nid);

		if(result==0)
		{
			AbortTransaction();
			printf("abort transaction update %d\n", i);
			return;
		}
	}
...
void TransactionTest(void)
{
	...
	printf("transaction: success=%d, sum=%d, abort=%d\n", success, sum, abort);
}
